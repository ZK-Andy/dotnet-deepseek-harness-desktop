...
{
    /// <summary>各 RID 的资产文件名后缀（发布命名契约：*_&lt;platform&gt;-&lt;arch&gt;.&lt;ext&gt;，见 release.yml/package 脚本）。</summary>
    private static readonly Dictionary<string, string> RidSuffixes = new(StringComparer.Ordinal)
    {
        ["linux-x64"] = "_linux-amd64.deb",
        ["linux-arm64"] = "_linux-arm64.deb",
    ...
    public static ReleaseMeta? Pick(string version, IEnumerable<string> hrefs, string rid, string repository)
    {
        if (!RidSuffixes.TryGetValue(rid, out var suffix))
        {
            return null;
        }
...
public sealed partial class ReleaseMetaClient
{
    ...
    public async Task<ReleaseMeta?> FetchLatestAsync(string rid, CancellationToken cancellationToken)
    {
        ...
        return ReleaseMeta.Pick(version, hrefs, rid, _options.Repository);
    }
