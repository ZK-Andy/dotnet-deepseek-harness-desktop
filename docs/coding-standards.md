...
- **CI 门禁**（ci.yml build-test）：`dotnet format --verify-no-changes --severity warn`——按 `.editorconfig` 校验，任何本配置会修掉的格式/风格差异（含已升 warning 的风格规则）即 fail loud。无谓 using（IDE0005）经此门禁强制：`.editorconfig` 已 `dotnet_diagnostic.IDE0005.severity = warning`。**注**：IDE0005 在 MSBuild 面需 `GenerateDocumentationFile` 才触发；测试工程开启该属性（其所有公共测试成员均已补 XML 文档），主工程不开启（Ryn.Callbacks 源生成器为 `[RynCallback]` 类产出 public DI 扩展，生成符号无法手写文档，且 `auto-generated` 头与 `.editorconfig` 路径 glob 均无法对编译期内存生成文件抑制 CS1591，全局 NoWarn 又会连带压制手写缺口——歧义，故不开启）。因此 IDE0005 的唯一强制路径是 `dotnet format` 门禁；主工程 csproj `NoWarn` 里的 `EnableGenerateDocumentationFile` 只压制 MSBuild 侧该指导性提示，不影响 format 门禁的 IDE0005 强制。
