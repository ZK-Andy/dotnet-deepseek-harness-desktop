...
* 覆盖面（按域分组，逐类细节见各文件头 `<summary>`）：
  ...
  * **随包插件**：`MarketInstallHelperTests`（检测/迁移/workspace 修正/spec 三分支/JsonNode 写盘格式钉子）、`PluginVersionCheckTests`（版本解包/升级判定/脏版本 fail loud）、`BundledPluginCatalogTests`（清单装配判定/端到端布局）。
...
* **打包自检**：`bundle-runtime-ci.sh` 的 `60s` 常驻抓 `dsh web:`（闭包签名含 dsh/node/companion/market/trimPolicy/scriptSha256 六维，任一漂移即全量重建）；`package-linux.sh --stage-only` 校验 `node + dsh/lib/bin.js + dshmarket.tgz`。
