...
- **插件市场**：推荐 `dsh-market`（`https://github.com/dsh-market/dsh-market`）——`DSH_HOME=~/.local/share/DeepSeek.Harness.Desktop/dsh dsh plugin add dshmarket` 一键安装后 `Web UI` 即有可视化商店，`1200+` 插件可搜一键装；`0.1.8` 的自动预装因首启阻塞已回退为手动，后续将改为后台非阻塞预装。
