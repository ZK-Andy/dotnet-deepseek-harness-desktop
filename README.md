...
- **插件市场预装**：`dsh-market`（`https://github.com/dsh-market/dsh-market`）已随包预装到 `resources/runtime`（`dshmarket.tgz`）——首启后台 `file://` 静默安装到 `DSH_HOME`，`1200+` 插件可搜一键装，无需命令行（`0.1.8` 的首启阻塞已修复为后台非阻塞）。
