...
- **Plugin ecosystem** — plugins install into the user's `DSH_HOME` profile (`dsh plugin --profile web add …`), including [dsh-market](https://github.com/dsh-market/dsh-market) one-click installs; `DSH_DESKTOP_PATCH` adds a desktop-side `--patch` overlay.
