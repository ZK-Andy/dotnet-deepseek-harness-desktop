...
- **Plugin market** — `dsh-market` (`https://github.com/dsh-market/dsh-market`) is recommended: `DSH_HOME=~/.local/share/DeepSeek.Harness.Desktop/dsh dsh plugin add dshmarket` gives the visual store for `1200+` plugins; `0.1.4` auto-preinstall was rolled back due to first-launch blocking and will return as a background install.
- **Native icon** — `assets/icon.png` (`DeepSeek Harness`-style `512` dark `DSH`) ships with the package, installed to `hicolor` and referenced as `Icon=deepseek-harness-desktop` in the `.desktop` entry, so the launcher, taskbar, and window share the same icon.
