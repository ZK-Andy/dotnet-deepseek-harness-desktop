...
- **Plugin market pre-installed** — `dsh-market` (`https://github.com/dsh-market/dsh-market`) ships with the app: the visual store appears on first launch for browsing/searching `1200+` plugins with one-click install, no CLI required; further plugins still live in `DSH_HOME`, and `DSH_DESKTOP_PATCH` can layer additional `--patch` overlays.
- **Native icon** — `assets/icon.png` (`pilot-harness`-style `512`) ships with the package, installed to `hicolor` and referenced as `Icon=deepseek-harness-desktop` in the `.desktop` entry, so the launcher, taskbar, and window share the same icon.
