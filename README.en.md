...
> Status: `v0.1.12` (`28/28` tests `24.3%` coverage + gates green, `CI` dual-green) — native shell + bundled runtime + crash recovery + market closed; `Wayland`/icon fixed, `0.1.11` ships `497K` real `tgz` + background `JSON` detection/migration/`allowBuilds` self-heal, `0.1.12` auto-restarts to show market.
...
- **Testable host layer** — `HarnessRuntimeHost` / `RuntimeSupervisor` / `RuntimeLocator` / `MarketInstallHelper` / URL parser are xunit `28/28` (`MarketInstallHelper 84%`) and gate-checked; `package-linux.sh` `fail loud` verifies real `tgz` in `staging`.
