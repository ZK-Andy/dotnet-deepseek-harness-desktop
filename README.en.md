...
<p align="center">
  ...
  <a href="https://github.com/ZK-Andy/dotnet-deepseek-harness-desktop/actions/workflows/ci.yml"><img src="https://img.shields.io/badge/tests-678%2F678-brightgreen" alt="tests"></a>
  <a href="docs/testing.md"><img src="https://img.shields.io/badge/coverage-57.52%25-yellowgreen" alt="coverage"></a>
...
- 🔄 **Crash self-heal & session return** — the shell supervises the runtime process: crash → recovery screen (cause / stderr tail + export diagnostics + exit) → auto-restart → same window back to a new URL; **the port stays stable** so the Web UI origin (and page-level session memory) survives — **return to your previous conversation after a crash or restart**. A restart after a plugin-market update uses **adopt-first** market relay: the successor the market brings up is adopted directly — one process, one navigation, no doomed competing launch or tens-of-seconds recovery screen. **Exception**: when the preferred port is held by a program the app cannot attribute to itself (another program, a dsh from another profile, or a leftover dsh whose ownership cannot be proven), that launch drifts to a new port — the origin changes and page-level session memory is no longer kept; if the drift happens after the page has loaded (crash recovery, market-relay restarts), the Update, Launch at sign-in, Minimize to tray on close and Export diagnostics controls in Settings fail for that session too — restart the app to recover.
...
>
> **Upgrade note**: the desktop profile directory is `profiles/dotnet-desktop` — on upgrade the old `profiles/desktop` is renamed automatically on first launch (plugins and port memory are kept; no manual step); use `dsh --profile dotnet-desktop` in the terminal. See [docs/user-guide.en.md](docs/user-guide.en.md).
>
