...
- **Plugin market** — `dsh-market` (`https://github.com/dsh-market/dsh-market`) is recommended: `DSH_HOME=~/.local/share/DeepSeek.Harness.Desktop/dsh dsh plugin add dshmarket` gives the visual store for `1200+` plugins; `0.1.8` auto-preinstall was rolled back due to first-launch blocking and will return as a background install.
