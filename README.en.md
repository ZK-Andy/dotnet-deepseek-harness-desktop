...
- **Plugin market pre-installed** — `dsh-market` (`https://github.com/dsh-market/dsh-market`) ships bundled in `resources/runtime`: the visual store appears on first launch for `1200+` plugins with one-click install, no CLI required (`0.1.4` blocking fixed by bundling the market into the closure).
