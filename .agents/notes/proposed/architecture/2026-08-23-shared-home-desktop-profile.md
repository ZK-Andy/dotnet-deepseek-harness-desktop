...
- 首启供给链的失败面（网络代理、pnpm/npm 政策拦截如 minimumReleaseAge、写入权限）：实现期逐项盘点，任何一步失败必须 fail loud 给出指引，禁止静默半装状态。供给路径是唯一路径：首启、自更新跨代（捆绑壳 → 去捆绑壳）、其他任何「找不到运行时」的场景共用同一条供给流程，**不设换代特殊机制**（前置验证、旧闭包回退均属过度设计，否决）。
