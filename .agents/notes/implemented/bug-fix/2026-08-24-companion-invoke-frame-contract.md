...
- **0.0.10 勘误（真机复验发现）**：归一化改名 `parseState→parseFrame` 时漏改 UpdateSection 一处引用，`ReferenceError` 走进 catch 使该区块永久显示「不可用」——`node --check` 不查未定义标识符。纪律补丁：改闭包内助手名必须全文件 grep 旧符号清零后再提交；「设置页各状态渲染」的真机验收项自此才算真正闭环过一次。
