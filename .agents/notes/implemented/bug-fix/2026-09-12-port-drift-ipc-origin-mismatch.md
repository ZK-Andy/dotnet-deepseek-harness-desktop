...
- 验证：`node --check` 通过；`dotnet test` 569/569、build 0 警告、`dotnet format` 与全部 `verify-*.py` 门禁绿。**实机（2026-09-12）**：同一台机器上，origin 错配的 0.4.8 会话（允许源钉 `:36111`、页面已漂到 `:44867`）设置页 invoke 全被拦；**重启后 origin 一致（`:44867`）→ 设置页查询与 `desktop.update.install` invoke 均可用、自更新 0.4.8→0.4.10 全链成功**（host.log 05:21:52 检查→下载→校验→ready，05:22:57 installing，05:23:33 新版启动）、companion 经版本感知升级到 0.0.18——既确证根因，也确证「重启恢复」这一处置。**待跑**：窗口已建后制造端口漂移时，设置页应显示「无法连接桌面宿主：本会话的页面命令通道不可用，重启应用可恢复自更新」而非 dev 文案。
- 用户文档对账：`README` 双语写出决策 2 的两组触发条件（任意漂移 → origin 变、页面级会话记忆不保留；页面已加载后漂移 → 设置页命令面失效）；`docs/faq{,.en}.md` 与 `docs/user-guide{,.en}.md` 的会话回归表述并入合并形态的「漂移可能使设置页命令面失效、重启应用恢复」。
