...
- **C｜不再收养续任者，改由壳自 spawn**：检出市场 helper 即收割，由壳自 spawn，等自家 dsh 给出 URL 后按 token URL 导航（冷启动路径，新 URL 天然绕缓存）；代价是 ~40s 恢复窗。**复访触发 = A/B 的实机验收结论**——塌陷仍落在用户可见窗口内，或稳定窗把收养推迟到不可接受，则评估 C。
  ...
  远期参照（2026-09-25 竞品调研）：`dsh-tauri/deepseek-harness-desktop` 用生产代码验证了同一方向——市场点重启只调 `window.dshDesktop.restartSidecar()`，重启权恒在壳，从无双重启者抢端口问题；且实现更简单（其 `restart.ts` 仅 11 行，无收养/稳定窗/免导航三层）。收养路径若长期走通则 C 永不转正；复访时以此为"更简单且已验证"的备选起点，省论证。
