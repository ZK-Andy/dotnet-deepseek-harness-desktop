...
- 坏页进门（smoke-settle-lib 落定分支）：日志出现 `鉴权页自愈失败` 即落定也判 FAIL（确认坏页机器可判，不再只靠人眼）；该字面门已由 [`page-verdict-gate`](../testing/2026-09-26-page-verdict-gate.md) 收敛为终页裁决行 `[nav] 页面裁决=auth`。
- 内容留痕（产品）：`SettleWebSessionAsync` 全路径 loud——每轮落定只写一条终页裁决行（origin + 可见文本长度，三态各一行文案）。长度不记内容，防敏感落盘。
...
- 绿 verdict 由到达门语义升级为终页裁决门（[`page-verdict-gate`](../testing/2026-09-26-page-verdict-gate.md)：显示腿须见 `页面裁决=healthy`）+ 每轮绿有尾日志 + 有截图——"绿+UI"结论可逐轮实证，不再靠推断。
- 代价：绿跑 CI 日志 +30 行；悬空池线程（hang 场景）；cookie 时好时坏的真病灶仍在（下批：P0 自愈触发证据齐后，主攻 token→cookie 链稳定性）。
- Ryn#101 关闭认错（见 Testing）。

## Testing

- 自测：裁决 auth 落定转失败用例（`settle-auth-fails`）；Task.Run/内容行/留尾沿既有先例（编排薄层不单测，配置矩阵已覆盖超时键）。
