...
- 实机验收 ✅（2026-09-25 21:55，v0.5.6 真机，插件开关往返触发的市场重启，取证 `~/.dsh/logs/host.log`）：`21:55:20` 子进程退出 → 同秒页内自刷到达一次 → `21:55:22` 接管（稳定窗满）→ `重启成功 →` 紧跟 `跳过壳侧导航`，其后无第二次到达、无 `dead`、无有界恢复；对照修复前 20:00 周期（两次到达：20:00:57 页内 + 20:01:00 壳侧）。用户体感：单次刷新、恢复黑屏不可见、整体变快（省掉第二次导航往返）。
