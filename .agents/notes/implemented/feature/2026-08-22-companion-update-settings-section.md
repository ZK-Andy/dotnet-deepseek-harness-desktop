...
- 失败降级（自更新栈未装载或命令通道失效）：`getState` invoke 失败 → 区块页内显示提示行，导航项保留（用户拍板页内提示而非整项隐藏）；两种成因分别出文案——dev 门禁（路由未注册）与命令通道整体失效（如端口漂移后的 [IPC origin 错配](../bug-fix/2026-09-12-port-drift-ipc-origin-mismatch.md)），用无条件注册的桌面命令探同一通道来判别。
