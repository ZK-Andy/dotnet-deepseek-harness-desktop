...
1. **探针住在宿主不在插件**：`PageHealthMonitor` 由壳侧定时对 WebView 跑一条只读表达式（body 的可见文本长度 > 0 即 alive），不注入脚本、不改页面、零 IPC——历史上 companion 自身就是事故源，探针住进插件会在最需要它的时候一起死。
2. **判定核心纯逻辑可单测**：`PageHealthTracker` 连续 3 次 Dead 才宣告迁移（导航空窗与渲染间隙去抖）；Unknown 不计数也不清零；任意 Alive 立即复位。锚点用 body 的可见文本长度而非 dsh 内部组件结构——上游 DOM 演进不致探针失效。
...
- **探针锚定 dsh UI 根组件选择器**：落败——上游前端演进会漂移选择器制造假阴性；body 的文本内容由渲染引擎产出，是引擎层事实。
- **MutationObserver 常驻监听**：落败——常驻观察者的内存与 CPU 开销换不来比 10s 轮询更多的信息量，观测场景无需实时。
- **参照逐点等同的精确 boot 花屏信号**：落败——参照（iframe 模型）精确识别 `#root` 下「HARNESS + Loading plugins」花屏才报 stalled；我方（直载模型）以「body 无可见文本即空白」为 Dead 信号，两者都捕获「进程在跑但页面没到应用态」的假活形态，恢复机致一致（有界 reload + leave 观测面），无需逐点模拟参照信号。
...
- [relay-restart-client-module-collapse](../bug-fix/2026-09-19-relay-restart-client-module-collapse.md)：其 A 条把本探针锚点由 body 子节点数改为可见文本长度；阈值/预算/Alive 复位语义仍以本篇为准。
