...
- **base 比对做空白归一化（正则仍严格）**：落败——它与匹配正交、不会放行扰动行（扰动行在正则一步即被拒，到不了 base 比对），反而会把「就地规范化该行」的补救判成非新产；净代价是把「逐字相等」降为空白等价类，两侧判据不再共用同一个可读契约，收益为负。
...
- 代价：base 比对取整篇笔记，因此一篇笔记若在 base 版本的**正文**里逐字引用了与自己头区相同的证据行（把真实 `Review:` 行当示例抄进正文），它后续合法写入的同文本头区行会被判非新产。按 fail-closed 拦下：改用 `#N` 或让示例不逐字等同即可。全仓证据行全部位于头区，无此形态。
- 覆盖缺口面：规范形正则拒绝一切双空格/字节扰动的 `FULL` 证据行，故头区存在此类行的笔记无法为该批提供证据（历史先例 `3230a5d`/`9853d5a` 的复算见 Testing）。全仓当前不存在此类行：`2026-09-12-coverage-baseline-from-ci-cobertura.md` 头区为规范形 `Review: FULL/2026-09-19#2/R1=ok R2=ok R3=ok`（三篇 `LIGHT/...` 行本就非 `FULL` 证据面）。故「起点早于某扰动行、范围内又无新产规范证据行」的 `--since` 假红面无当前可实现路径；本仓直推 main，pre-push 的 `origin/main` 与 push 事件的 `event.before` 都随最新批前移。
