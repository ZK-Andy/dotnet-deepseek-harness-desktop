...
- **串行 + 有界（简报定界）**：R1→R2→R3 依次，前一个收口才放下一个；每路任务由主会话简报定界（范围 + 定向检查项 + 不做清单），一次完整跑完；**无「到限中断取部分结论」出口**（中断即未审计，中断后须重新完整跑——见 [review-scope-narrowing](2026-08-31-review-scope-narrowing.md) 第 4 节）。
