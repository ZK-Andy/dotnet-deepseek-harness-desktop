...
2. **Linux 安装冒烟**（`scripts/smoke-install-linux.sh`）：deb 在 runner 原生安装（apt 解依赖）、rpm 在 fedora 容器内安装（dnf 解依赖），随后启动已安装的二进制，等待窗内以双信号判 PASS（命中其一）：`[host] dsh web =`（全链：装包→引导→dsh web 就绪）或 `[bootstrap] 引导开始：`（安装链：CI 无显示环境壳在 Ryn Run 即退出属已记录边界，引导后台任务随之夭折，全链信号在 CI 不可达）。online-first 批次二起判定链覆盖「装包 → 首启引导 → dsh web URL」全链（包内无闭包，应用先引导安装运行时）；等待窗与引导步超时强耦合（默认 2×`StepTimeoutMinutes`+余量，`SMOKE_WAIT_SECONDS` 可覆写）。两架构 matrix 各自原生执行；进程探活用 `kill -0 <pid>` 而非 pgrep（安装后入口是小写符号链接，按大写二进制名 pgrep 会立即误判进程已死）。
