...
- 代价/风险：插件安装检测对 dshmarket 只查存在性不比版本（companion 已由版本感知升级机制按 bundled tgz 版本比对自动升版）；client 侧 JS 无自动化测试（薄监听 + 判定逻辑留在 C# 缓解）；dsh web 未启动时插件不生效（可接受——接管对象本就是 dsh 页面）；`shell.overlay`/boot manifest 为上游非契约表面，靠逐 release 钉死内置 dsh 版本兜底；客户端半的 `apply` 契约是 web boot 的硬门槛（见 Decision），任何后续手改 client.js 都不得让 factory 返回无 `apply` 的对象。
