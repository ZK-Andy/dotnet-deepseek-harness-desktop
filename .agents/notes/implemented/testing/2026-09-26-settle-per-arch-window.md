...
arm64 原生建窗 100s+（dbus 未治愈；托盘错误变形证明总线已存在，建窗仍慢——根因仍在 GTK/WebKit 初始化深处，Ryn#101 跟进中），而落定窗全腿 90s：hop1 在 ①后约 70–100s 才发起，90s 窗内等不到提交即收工——等的一方先超时，不是不通。amd64 建窗快（hop1 在 ①后秒级），窗本可更短。全腿统一加长会拖慢失败反馈；只给慢腿加长是精准匹配。

## Decision

- 矩阵加 `settle_seconds`：amd64 180 /arm64 300（amd64 窗随终页裁决门加长，见 [`page-verdict-gate`](2026-09-26-page-verdict-gate.md)：窗口须覆盖两跳超时+探针预算的最坏链），经既有 `SMOKE_SETTLE_SECONDS` 口透传（脚本 `SETTLE_WAIT` 默认 90 不动，rpm 容器腿自动继承——容器恒②，无窗可落定，值变无影响）。
