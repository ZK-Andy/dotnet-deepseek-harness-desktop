...
- 范围：第二跳保持原生（http→http 是否 hang 未证，先看本批对照结果再定）；mac/win 不动。

## Alternatives considered

- **继续加落定窗/探针预算**：落败——两病灶分别死在"见证没跑"与"导航没发出"，加时间只延长红。
- **等 Ryn 修原生 hang**：落败——#101 已因错误模型关闭，重开+最小复现+等上游是周级远水；eval 绕行是宿主侧当天可验证的对照。
...
- 新增 `PagePumpEvalNavigateTests`（缝为 eval 委托，无 WebView）：脚本纯函数（URL 转义/`try` 包裹/同步布尔）；`true→true`、`false→false`、悬空→有界 `false`、抛异常→`false`、已取消 token→上抛。
- `bash scripts/smoke-install-linux.sh --self-test`（含既有 witness 白/深/UI 三夹具）。
- `dotnet test` 全绿对基线；dispatch `package-linux` 看 amd64 见证行 + arm64 到达门。
