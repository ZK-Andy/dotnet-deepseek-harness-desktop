...
1. **读 HANDOFF 家庭**——读 `HANDOFF.md`「**状态区**」（背景/位置/当前状态/待办/开始步骤）+「**交接更新记录**」摘要滚动窗；**待办明细读 `HANDOFF-todos.md`**（`[ ]` 条为行动清单、`[x]` 为近期窗口的一行指针，越窗指针在 `.plan/todos-archive/` 冷归档卷）；如需近期过程细节，再读 `.plan/journal/` 对应**月卷**（如 `2026-08-session-journal.md`，按月归档）。踩坑判别要点见 `docs/cookbook.md`（原 Gotchas 已并入，HANDOFF 不再承载）。
