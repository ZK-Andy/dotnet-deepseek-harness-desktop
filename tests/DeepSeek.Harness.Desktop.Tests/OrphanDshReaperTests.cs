...
public class OrphanDshReaperTests
{
    ...
    {
        string dir = Path.Combine(Path.GetTempPath(), "dsh-reaper-" + Guid.NewGuid().ToString("N"));
    ...
    public void Reap_TokenMatches_KillsTree()
    {
        ...
        try
        {
            ...
            bool reaped = OrphanDshReaper.Reap(
                pidPath,
                readToken: _ => "abc123",
                killTree: p => { killed = p; },
                log: _ => { });
    ...
    public void Reap_TokenUnreadable_DoesNotKill()
    {
        ...
        try
        {
            string pidPath = WriteSpawnRecord(dir, 4242, "token");
            int killed = 0;
    ...
    public void Reap_PidFileMissing_ReturnsFalse()
    {
        ...
        try
        {
            string missing = Path.Combine(dir, "does-not-exist");
            bool reaped = OrphanDshReaper.Reap(
                missing,
                readToken: _ => "token",
                killTree: _ => { },
    ...
    public void Reap_PidFileCorrupt_ReturnsFalse()
    {
        ...
        try
        {
            string pidPath = Path.Combine(dir, ".dsh-pid");
