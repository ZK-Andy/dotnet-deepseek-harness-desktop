...
public class DesktopProfileBootstrapTests
{
    ...
    public void EnsureProfile_FreshHome_CreatesManifestWithWebAppBundles_AndSiblingFiles()
    {
        ...
        try
        {
            ...
            string manifest = File.ReadAllText(Path.Combine(dir, "package.json"));
    ...
    public void ReconcileProfile_RemovesDeadLocalSeedRefs_KeepsRegistryAndPresentLocal()
    {
        ...
        try
        {
            ...
            string aliveDir = Path.Combine(dir, "alive-pkg");
            ...
            int removed = DesktopProfileBootstrap.ReconcileProfile(home, logs.Add);
    ...
    {
        string dir = Path.Combine(Path.GetTempPath(), "dsh-bootstrap-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(dir);
        return dir;
    }
}
